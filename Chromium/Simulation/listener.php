...
$user_file = "Z:\\user.txt";
$db_file = "Z:\\users.db";
$lastcmd = "";
$buffer = array();
$input_buffer = array();
	echo "Listening...";
	$server = new SocketServer("localhost",7775); // Create a Server binding to the given ip address and listen to port 7775 for connections
...
{
	global $db_file;
	//in reality, send username and password to the FPGA
	if (strlen($username) <= 16 && strlen($password) <= 16) //max: 16 chars for each
	{
		return file_put_contents($db_file, $username."\n".$password."\n", FILE_APPEND);
	}
	return false;
}
...
{
	global $user_file;
	//in reality, send "SET" to Linux communication channel
	return file_put_contents($user_file, $username . "\n".rand_str());
}
function GetCurrentAccount()
{
	global $user_file;
	//in reality, send "GET" to Linux communication channel and return response
...
{
	global $db_file;
	//in reality, send "LIST" to Linux communication channel and retrieve list of usernames
	$fhandle = fopen($db_file,"r");
	$i = 0;
	$accounts = [];
...
{
	SocketServer::socket_write_smart($client->socket, $response);
}
function handle_connect(&$server,&$client,$input)
{
	$client->uid = $client->socket.uniqid();
	echo "Connected\r\n";
}
function process_input($input, $client)
{
	global $lastcmd, $input_buffer;
	$trim = trim(str_replace("\x00", "", $input));
	if ($lastcmd == "add")
	{
		$input_buffer[] = $trim;
		if (count($input_buffer) >= 2)
		{
			AddAccount($input_buffer[0], $input_buffer[1]);
		}
		else
		{
			$lastcmd = "add";
	...
	{
		SetCurrentAccount($trim);
	}
	else if(strtolower($trim) == "quit") // User Wants to quit the server
    {
        SocketServer::socket_write_smart($client->socket,"Oh... Goodbye..."); // Give the user a sad goodbye message, meany!
        $server->disconnect($client->server_clients_index); // Disconnect this client.
        return; // Ends the function
    }
	else if (strtolower($trim) == "list")
	{
		SendResponse($client, json_encode(ListAccounts()));
	}
	else if (strtolower($trim) == "get")
	{
		SendResponse($client, json_encode(GetCurrentAccount()));
	}
	$lastcmd = strtolower($trim);
}
...
{
	global $buffer;
	if (!isset($buffer[$client->uid])) $buffer[$client->uid] = [];
	if (strpos($input, "\n") === false)
	{
		$buffer[$client->uid][] = $input;
	}
	else
	{
		preg_match("/(.*?)\n/", $input, $matches);
		$buffer[$client->uid][] = trim($matches[1]);
		process_input(implode("", $buffer[$client->uid]), $client);
		$buffer[$client->uid] = [];
	}
