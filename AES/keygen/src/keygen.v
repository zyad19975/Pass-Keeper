...
module keygen(round_num,keyin,keyout);
   ...
   wire [0:31] block1 ,block2,block3,block4,dummy;
         
               
       
       
       
		assign block1 = keyin[0 : 31];					//		| b1 | b2 | b3 | b4 |
		assign block2 = keyin[32: 63];					//		| b1 | b2 | b3 | b4 |
		assign block3 = keyin[64: 95];					//		| b1 | b2 | b3 | b4 |
		assign block4 = keyin[96:127];					//		| b1 | b2 | b3 | b4 |	
	   ...
	   // assign output						  
       assign keyout[0  : 31] = block1 ^ dummy ^ rcon(round_num);
       assign keyout[32 : 63] = block1 ^ dummy ^ rcon(round_num)^ block2;
       assign keyout[64 : 95] = block1 ^ dummy ^ rcon(round_num)^ block3 ^ block2;
       assign keyout[96 :127] = block1 ^ dummy ^ rcon(round_num)^ block4 ^ block3 ^ block2;
